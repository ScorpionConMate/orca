...
export function registerFilesystemWatcherHandlers(): void {
  ipcMain.handle('fs:watchWorktree', async (event, rawArgs: unknown): Promise<void> => {
    ...
    // Why: own sender teardown from the instant we hold a claim, so every early return below
    // (provider unavailable + retry, renderer destroyed mid-install, throw) still releases the
    // admission via releaseSender — otherwise a destroyed WebContents leaks its claim until shutdown.
